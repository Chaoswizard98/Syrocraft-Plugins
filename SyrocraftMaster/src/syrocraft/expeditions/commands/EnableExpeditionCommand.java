...
public class EnableExpeditionCommand implements CommandExecutor{
	Main plugin;
	public EnableExpeditionCommand(Main pl){
		this.plugin = pl;
	}
			...
			if(commandLabel.equalsIgnoreCase("EnableExpedition")){//EnableExpedition <Expedition>
				if(args.length == 1){
					...
					if(f.exists()){
						...
						theSender.sendMessage(ChatColor.GREEN + "Successfully enabled expedition.");
					}
					else{
